...
describe ApplicationController, type: :request do
  ...
  describe 'Skylight link' do
    ...
    context 'when SKYLIGHT_PUBLIC_DASHBOARD_URL is set' do
      ...
      it 'includes a link to view performance data' do
        get '/'
    ...
    context 'when SKYLIGHT_PUBLIC_DASHBOARD_URL is not set' do
      ...
      it 'does not include a link to view performance data' do
        get '/'
        expect(response.body).not_to match(/performance data/i)
        expect(response.body).not_to match(/skylight/i)
      end
    end
  end
end
