...
class Track < ApplicationRecord
  ...
  def dates_within_conference_dates
    ...
    unless (program.conference.start_date..program.conference.end_date).cover?(start_date)
      errors.add(:start_date, "can't be outside of the conference's dates (#{program.conference.start_date}-#{program.conference.end_date})")
    end
    unless (program.conference.start_date..program.conference.end_date).cover?(end_date)
      errors.add(:end_date, "can't be outside of the conference's dates (#{program.conference.start_date}-#{program.conference.end_date})")
    end
  end
  ...
  def overlapping
    ...
    (program.tracks.accepted + program.tracks.confirmed - [self]).each do |existing_track|
      unless existing_track.room == room && existing_track.start_date && existing_track.end_date
        next
      end
